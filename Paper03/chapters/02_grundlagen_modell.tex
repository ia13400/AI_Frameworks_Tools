...
Pythia-410M ist ein offenes, vollständig dokumentiertes Decoder-Only-\gls{transformer}-Modell (GPT-NeoX-Architektur) von EleutherAI mit rund 405 Millionen Parametern \cite{biderman2023pythia}. Tabelle~\ref{tab:arch} fasst die zentralen Architekturwerte zusammen.
...
Innerhalb einer Schicht entfällt etwa ein Drittel der Parameter auf \gls{attention} und zwei Drittel auf das Feed-Forward-Netzwerk (Abbildung~\ref{fig:params}); der größte Teil der Modellkapazität liegt also nicht in der Attention selbst, sondern in den Feed-Forward-Blöcken.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{parameterverteilung.png}
  \caption{Parameterverteilung von Pythia-410M auf Embedding, Attention, MLP und Unembedding.}
  \label{fig:params}
\end{figure}
